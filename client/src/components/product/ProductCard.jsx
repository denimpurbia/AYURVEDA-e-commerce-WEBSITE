...
const ProductCard = ({ product }) => {
  ...
  const image = product.images && product.images.length > 0 ? product.images[0] : 'https://images.unsplash.com/photo-1540420773420-3366772f4999?auto=format&fit=crop&q=80&w=600';
    ...
    <div className="group bg-[#FFFDF8] rounded-2xl border border-[#EAE1D2] overflow-hidden shadow-soft hover:shadow-card transition-all duration-300 flex flex-col justify-between">
      ...
      <div className="relative aspect-square bg-[#F7F2E8] overflow-hidden p-4">
        ...
        <button
          ...
          aria-label="Wishlist"
        >
          <Heart className={`w-4 h-4 ${isLiked ? 'fill-[#C49A52] text-[#C49A52]' : ''}`} />
        </button>
