      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
        ...
        {items.length === 0 ? (
          <div className="bg-[#F7F2E8] p-12 text-center rounded-3xl border border-[#EAE1D2] space-y-4 max-w-lg mx-auto">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#123D2A]">Your Cart is Empty</h2>
            ...
            >
              START SHOPPING <ArrowRight className="w-4 h-4 text-[#C49A52]" />
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="bg-[#FFFDF8] rounded-2xl border border-[#EAE1D2] divide-y divide-[#EAE1D2]">
                {items.map((item) => {
                  ...
                  const name = product.name || item.name || 'Ayurvedic Product';
                  const price = item.price;
                  const image = (product.images && product.images[0]) || product.image || 'https://images.unsplash.com/photo-1540420773420-3366772f4999?auto=format&fit=crop&q=80&w=600';

                  return (
                    <div key={productId} className="p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="flex items-center space-x-4">
                        <img src={image} alt={name} className="w-20 h-20 object-contain rounded-xl bg-[#F7F2E8] p-2" />
                        <div>
                          <h3 className="font-serif font-bold text-sm text-[#123D2A]">{name}</h3>
                          <span className="text-xs text-[#7A6248] font-bold block mt-1">₹{price}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-6">
                        ...
                        <div className="flex items-center border border-[#EAE1D2] rounded-full bg-[#F7F2E8]">
                          ...
                          <span className="px-3 text-xs font-bold text-[#123D2A]">{item.quantity}</span>
                          <button
              ...
              <div className="flex justify-between items-center pt-2">
                <Link to="/shop" className="text-xs font-bold text-[#123D2A] hover:underline">
                  ← Continue Shopping
                </Link>
                <button
                  onClick={clearCart}
            ...
            <div className="lg:col-span-4 bg-[#F7F2E8] p-6 rounded-2xl border border-[#EAE1D2] space-y-6">
              <h3 className="font-serif font-bold text-xl text-[#123D2A]">Order Summary</h3>
              ...
              {subtotal < 999 && (
                <div className="p-3 bg-[#FFFDF8] rounded-xl border border-[#789B72]/40 text-xs text-[#123D2A] flex items-center gap-2">
                  ...
                  <span>Add <strong>₹{999 - subtotal}</strong> more for <strong>FREE SHIPPING</strong>!</span>
                </div>
              )}
              ...
              <div className="space-y-3 text-xs font-medium text-[#243229]">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span className="font-bold text-[#123D2A]">₹{subtotal}</span>
                </div>
                <div className="flex justify-between">
              ...
              >
                PROCEED TO CHECKOUT <ArrowRight className="w-4 h-4 text-[#C49A52]" />
              </button>
            </div>
