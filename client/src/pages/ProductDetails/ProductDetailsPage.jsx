    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          try {
            const revRes = await API.get(`/product-reviews/product/${res.data._id}`);
            if (revRes.success) {
    ...
    <div className="min-h-screen bg-[#FFFDF8] flex flex-col justify-between">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
        ...
        <div className="space-y-6">
          ...
          {reviews.length === 0 ? (
            <div className="p-6 bg-[#F7F2E8]/60 rounded-2xl border border-[#EAE1D2] text-center max-w-lg">
              <p className="text-xs font-semibold text-[#7A6248]">
                No customer reviews yet for this formulation.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {reviews.map((rev) => (
                <div key={rev._id} className="p-5 bg-[#F7F2E8]/60 rounded-2xl border border-[#EAE1D2] space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-bold text-xs text-[#123D2A] block">
                  ...
                  <p className="text-xs text-[#243229]/80 leading-relaxed italic">
                    "{rev.comment}"
                  </p>
                      ...
                      {rev.images.map((img, idx) => {
                        ...
                        return (
                          <img
                            ...
                            src={imgUrl}
                            alt="Review photo"
                            className="w-16 h-16 object-cover rounded-xl border border-[#EAE1D2]"
                          />
                        );
                      })}
                    </div>
